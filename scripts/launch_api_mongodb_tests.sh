...
echo "Executing api-mongodb tests in container $CONTAINER_NAME"
...
docker exec $CONTAINER_NAME python3 -m pytest \
        ...
        --cov-report term \
        --rootdir /app && \
docker cp $CONTAINER_NAME:/app/test_result.xml $RESULT_FOLDER/api-mongodb_test_result.xml  && \
docker cp $CONTAINER_NAME:/app/test_coverage.xml $RESULT_FOLDER/api-mongodb_test_coverage.xml