...
# Monitoring
To ensure the infrastrcuture is working correctly, monitoring tools have been included in the project as described below.

## Prometheus
Available at [https://localhost:9090](https://localhost:9090), Prometheus is responsible for collecting metrics related to the services and for generating alerts under pre-defined conditions.

## AlertManager
Available at [https://localhost:9093](https://localhost:9093), AlertManager is responsible for processing the alerts generated by Prometheus and for routing as configured or for silenting them if required.

Several ways to route alerts are provided by AlertManager and in this project we decided to use Slack notification to the `#avr25-mle-rakuten-alert` channel of the `AVR25 MLE Rakuten` workspace. The link to this channel is [here](https://avr25mlerakuten.slack.com/archives/C09G7BH9Q6R) and is only available to authorized users.

## Grafana
To visualize the infrastructure metrics, Grafana is available at [https://localhost:3030](https://localhost:3030) and include predetermined dashboards to display infrastructure related graphics.