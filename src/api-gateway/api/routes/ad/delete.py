...
async def delete_add_mongodb_action(token: str, uid: str, data: Dict[str, str]):
    ...
    response = mongodb_client.delete_add(token=token, user_data=uid, ad_data=data)
    return {"message": "Add deleted successfully from MongoDB", "response": response}


async def delete_add_postgresql_action(token: str, uid: str, data: Dict[str, str]):
    ...
    response = postgresql_client.delete_add(token=token, user_data=uid, ad_data=data)

    return {"message": "Add deleted successfully from PostgreSQL", "response": response}

@router.delete("/add/delete/{ad_id}")
async def delete_ad(user_data: dict = Depends(get_user_data_from_token), data= Dict[str, str]):
    ...
    mongodb_response = await delete_add_mongodb_action(mongodb_token, mongodb_uid, data)
    postgresql_response = await delete_add_postgresql_action(postgresql_token, postgresql_uid, data)
...
# @router.delete("/add/delete/mongodb")
# async def delete_add_mongodb(user_data: dict = Depends(get_user_data_from_token)):
#     mongodb_token = user_data.get('tokens', {}).get('mongodb')
#     mongodb_uid = user_data.get('uid', {}).get('mongodb')
...
# @router.delete("/add/delete/postgresql")
# async def delete_add_postgresql(user_data: dict = Depends(get_user_data_from_token)):
#     postgresql_token = user_data.get('tokens', {}).get('postgresql')    
#     postgresql_uid = user_data.get('uid', {}).get('postgresql')
#     if not user_id:
#         raise HTTPException(
