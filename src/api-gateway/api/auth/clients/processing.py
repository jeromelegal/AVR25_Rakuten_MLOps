...
class ProcessingClient:
    ...
    def _get_base_url(self) -> str:
        ...
        if base:
            ...
            return base
        scheme = self._get_env("API_PROCESSING_SCHEME", "http")
        host = self._get_env("API_PROCESSING_HOST", "api-processing")
        port = self._get_int("API_PROCESSING_PORT", 8050)
    ...
    def predict(self,
        ...
        # Backward-compat: allow signature predict(token, description, files)
        if files is None and isinstance(designation, list) and all(
            isinstance(x, (bytes, bytearray)) for x in designation
        ):
            files = designation
            designation = ""

        """Appelle l’endpoint externe de prédiction"""
        url = f"{self._get_base_url()}/api-processing/predict"
        headers = self.get_headers(token)
        session = self.get_session()

        nb_files = 0 if files is None else len(files)
        logger.debug(
            f"Calling processing service at {url} with description: {description}, "
            f"designation: {designation}, number of files: {nb_files}"
        ) 
        logger.debug(f"Headers: {headers}")

        files_payload = []
        if files:
            files_payload = [("files", ("file.jpg", f, "image/jpeg")) for f in files]

        data = {"description": description, "designation": designation or ""}

        try:
            r = session.post(url, headers=headers, data=data, files=files_payload, timeout=30)
            r.raise_for_status()
            return r.json()
