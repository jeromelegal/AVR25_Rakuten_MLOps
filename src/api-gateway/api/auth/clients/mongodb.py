...
class MongoDBClient:
    ...
    def get_session(self) -> Session:
        ...
        session = Session()
        adapter = MTLSAdapter(
            ca_path=self.settings.API_MONGODB_API_GATEWAY_CA_PATH,
            key_path=self.settings.API_MONGODB_API_GATEWAY_KEY_PATH,
            cert_path=self.settings.API_MONGODB_API_GATEWAY_CERT_PATH
        )
    ...
    def create_entity(self, token: str, collection: str, entity_data: Dict[str, Any]) -> Dict[str, Any]:
        """ Crée une nouvelle entité dans la collection spécifiée. """
        base_url = self.settings.API_MONGODB_BASE_URL
        headers = self.get_headers(token)
        session = self.get_session()

        endpoint = f"{base_url}/api/internal/mongodb/entity/{collection}"
    ...
    def read_entity(self, token: str, collection: str, entity_id: str) -> Dict[str, Any]:
        """ Lit une entité de la collection spécifiée. """
        base_url = self.settings.API_MONGODB_BASE_URL
        headers = self.get_headers(token)
        session = self.get_session()

        endpoint = f"{base_url}/api/internal/mongodb/entity/{collection}/{entity_id}"
        logger.debug(f"Request URL: {endpoint}")
    ...
    def update_entity(self, token: str, collection: str, entity_id: str, entity_data: Dict[str, Any]) -> Dict[str, Any]:
        """ Met à jour une entité dans la collection spécifiée. """
        base_url = self.settings.API_MONGODB_BASE_URL
        headers = self.get_headers(token)
        session = self.get_session()
    ...
    def delete_entity(self, token: str, collection: str, entity_id: str) -> Dict[str, Any]:
        """ Supprime une entité de la collection spécifiée. """
        base_url = self.settings.API_MONGODB_BASE_URL
        headers = self.get_headers(token)
        session = self.get_session()
