...
cat <<EOF > "${MLFLOW_API_IMAGE_PROCESSING_KEY_PATH}"
$(printf "%s" "$MLFLOW_API_IMAGE_PROCESSING_KEY")
EOF

cat <<EOF > "${MLFLOW_API_IMAGE_PROCESSING_CERT_PATH}"
$(printf "%s" "$MLFLOW_API_IMAGE_PROCESSING_CERT")
EOF

chown api-image-processing:api-image-processing $MLFLOW_API_IMAGE_PROCESSING_KEY_PATH
chmod 600 $MLFLOW_API_IMAGE_PROCESSING_KEY_PATH

# Extraire le certificat et la clé privée de Minio
MINIO_API_IMAGE_PROCESSING_CA=$(vault kv get -field=ca secret/minio/api-image-processing/certs)
MINIO_API_IMAGE_PROCESSING_CERT=$(vault kv get -field=cert secret/minio/api-image-processing/certs)
MINIO_API_IMAGE_PROCESSING_KEY=$(vault kv get -field=key secret/minio/api-image-processing/certs)

cat <<EOF > $MINIO_API_IMAGE_PROCESSING_PEM_PATH
$(printf "%s" "$MINIO_API_IMAGE_PROCESSING_KEY")
$(printf "%s" "$MINIO_API_IMAGE_PROCESSING_CERT")
EOF

cat <<EOF > $MINIO_API_IMAGE_PROCESSING_CA_PATH
$(printf "%s" "$MINIO_API_IMAGE_PROCESSING_CA")
EOF

cat <<EOF > "${MINIO_API_IMAGE_PROCESSING_KEY_PATH}"
$(printf "%s" "$MINIO_API_IMAGE_PROCESSING_KEY")
EOF

cat <<EOF > "${MINIO_API_IMAGE_PROCESSING_CERT_PATH}"
$(printf "%s" "$MINIO_API_IMAGE_PROCESSING_CERT")
EOF

chown api-image-processing:api-image-processing $MINIO_API_IMAGE_PROCESSING_KEY_PATH
chmod 600 $MINIO_API_IMAGE_PROCESSING_KEY_PATH