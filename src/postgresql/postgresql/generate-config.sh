...
for ip in $ips; do
    ...
    echo -e "hostssl\tall\t\t$AIRFLOW_USER\t$ip/16\t\tcert clientcert=verify-full" >> /etc/postgresql/pg_hba.conf
done
...
echo -e "local\tall\t\t$AIRFLOW_USER\t\t\t\tscram-sha-256" >> /etc/postgresql/pg_hba.conf
