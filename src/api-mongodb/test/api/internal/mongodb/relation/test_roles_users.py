...
async def test_create_role_user():
    async with get_db_client() as db:
        ...
        response = client.post("/api/internal/mongodb/relation/role_user", json={"role_id": role_id, "user_id": user_id, "permissions": {"read": True, "write": False}}, headers=headers)
        ...
        # Clean up
        await db.role_users.delete_one({"_id": ObjectId(data["relation_id"])})
...
async def test_get_role_user():
    async with get_db_client() as db:
        ...
        await db.role_users.insert_one({"_id": ObjectId(relation_id), "role_id": role_id, "user_id": user_id, "permissions": {"read": True, "write": False}, "created_at": "2023-10-01T00:00:00Z", "created_by": user_id})

        response = client.get(f"/api/internal/mongodb/relation/role_user/{relation_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["role_id"] == role_id
        assert data["user_id"] == user_id
        ...
        # Clean up
        await db.role_users.delete_one({"_id": ObjectId(relation_id)})
        await db.roles.delete_one({"_id": ObjectId(role_id)})
        await db.users.delete_one({"_id": ObjectId(user_id)})
...
async def test_update_role_user():
    async with get_db_client() as db:
        ...
        response = client.put(f"/api/internal/mongodb/relation/role_user/{relation_id}", json={"role_id": role_id, "user_id": user_id, "permissions": {"read": True, "write": True}}, headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["role_id"] == role_id
        assert data["user_id"] == user_id
...
async def test_delete_role_user():
    async with get_db_client() as db:
        ...
        response = client.delete(f"/api/internal/mongodb/relation/role_user/{relation_id}", headers=headers)
        assert response.status_code == 200
