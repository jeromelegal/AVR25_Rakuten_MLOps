...
@pytest.mark.asyncio
async def test_create_role_widget():
    async with get_db_client() as db:
        ...
        response = client.post("/api/internal/mongodb/relation/role_widget", json={"role_id": role_id, "widget_id": widget_id, "permissions": {"read": True, "write": False}}, headers=headers)
...
async def test_get_role_widget():
    async with get_db_client() as db:
        ...
        # Create a role-widget relation
        relation_id = str(ObjectId())
        await db.role_widgets.insert_one({"_id": ObjectId(relation_id), "role_id": role_id, "widget_id": widget_id, "permissions": {"read": True, "write": False}, "created_at": "2023-10-01T00:00:00Z", "created_by": user_id})

        response = client.get(f"/api/internal/mongodb/relation/role_widget/{relation_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["role_id"] == role_id
        assert data["widget_id"] == widget_id
        ...
        # Clean up
        await db.role_widgets.delete_one({"_id": ObjectId(relation_id)})
        await db.roles.delete_one({"_id": ObjectId(role_id)})
        await db.widgets.delete_one({"_id": ObjectId(widget_id)})
        await db.users.delete_one({"_id": ObjectId(user_id)})

@pytest.mark.asyncio
async def test_update_role_widget():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.put(f"/api/internal/mongodb/relation/role_widget/{relation_id}", json={"role_id": role_id, "widget_id": widget_id, "permissions": {"read": True, "write": True}}, headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["role_id"] == role_id
        assert data["widget_id"] == widget_id
...
@pytest.mark.asyncio
async def test_delete_role_widget():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.delete(f"/api/internal/mongodb/relation/role_widget/{relation_id}", headers=headers)
        assert response.status_code == 200
        assert response.json() == {"message": "Role-Widget relation deleted successfully"}

        # Verify deletion
        role_widget = await db.role_widgets.find_one({"_id": ObjectId(relation_id)})
        assert role_widget is None
