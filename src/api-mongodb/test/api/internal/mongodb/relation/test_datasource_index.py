...
@pytest.mark.asyncio
async def test_create_datasource_index():
    async with get_db_client() as db:
        ...
        response = client.post("/api/internal/mongodb/relation/datasource_index", json={"datasource_id": datasource_id, "index_id": index_id, "position": 1}, headers=headers)
...
async def test_get_datasource_index():
    async with get_db_client() as db:
        ...
        # Create a datasource-index relation
        relation_id = str(ObjectId())
        await db.datasource_index.insert_one({"_id": ObjectId(relation_id), "datasource_id": datasource_id, "index_id": index_id, "position": 1, "created_at": "2023-10-01T00:00:00Z", "created_by": user_id})

        response = client.get(f"/api/internal/mongodb/relation/datasource_index/{relation_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["datasource_id"] == datasource_id
        assert data["index_id"] == index_id
        ...
        # Clean up
        await db.datasource_index.delete_one({"_id": ObjectId(relation_id)})
        await db.datasources.delete_one({"_id": ObjectId(datasource_id)})
        await db.indexes.delete_one({"_id": ObjectId(index_id)})
        await db.users.delete_one({"_id": ObjectId(user_id)})

@pytest.mark.asyncio
async def test_update_datasource_index():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.put(f"/api/internal/mongodb/relation/datasource_index/{relation_id}", json={"datasource_id": datasource_id, "index_id": index_id, "position": 2}, headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["datasource_id"] == datasource_id
        assert data["index_id"] == index_id
...
@pytest.mark.asyncio
async def test_delete_datasource_index():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.delete(f"/api/internal/mongodb/relation/datasource_index/{relation_id}", headers=headers)
        assert response.status_code == 200
        assert response.json() == {"message": "Datasource-Index relation deleted successfully"}

        # Verify deletion
        relation = await db.datasource_index.find_one({"_id": ObjectId(relation_id)})
