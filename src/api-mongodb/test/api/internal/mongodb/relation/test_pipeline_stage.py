...
@pytest.mark.asyncio
async def test_create_pipeline_stage():
    async with get_db_client() as db:
        ...
        response = client.post("/api/internal/mongodb/relation/pipeline_stage", json={"pipeline_id": pipeline_id, "stage_id": stage_id, "position": 1}, headers=headers)
...
async def test_get_pipeline_stage():
    async with get_db_client() as db:
        ...
        # Create a pipeline-stage relation
        relation_id = str(ObjectId())
        await db.pipeline_stage.insert_one({"_id": ObjectId(relation_id), "pipeline_id": pipeline_id, "stage_id": stage_id, "position": 1, "created_at": "2023-10-01T00:00:00Z", "created_by": user_id})

        response = client.get(f"/api/internal/mongodb/relation/pipeline_stage/{relation_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["pipeline_id"] == pipeline_id
        assert data["stage_id"] == stage_id
        ...
        # Clean up
        await db.pipeline_stage.delete_one({"_id": ObjectId(relation_id)})
        await db.pipelines.delete_one({"_id": ObjectId(pipeline_id)})
        await db.stages.delete_one({"_id": ObjectId(stage_id)})
        await db.users.delete_one({"_id": ObjectId(user_id)})
...
async def test_update_pipeline_stage():
    async with get_db_client() as db:
        ...
        response = client.put(f"/api/internal/mongodb/relation/pipeline_stage/{relation_id}", json={"pipeline_id": pipeline_id, "stage_id": stage_id, "position": 2}, headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["pipeline_id"] == pipeline_id
        assert data["stage_id"] == stage_id
...
@pytest.mark.asyncio
async def test_delete_pipeline_stage():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.delete(f"/api/internal/mongodb/relation/pipeline_stage/{relation_id}", headers=headers)
        assert response.status_code == 200
        assert response.json() == {"message": "Pipeline-Stage relation deleted successfully"}

        # Verify deletion
        relation = await db.pipeline_stage.find_one({"_id": ObjectId(relation_id)})
