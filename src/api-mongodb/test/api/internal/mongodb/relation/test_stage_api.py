...
@pytest.mark.asyncio
async def test_create_stage_api():
    async with get_db_client() as db:
        ...
        response = client.post("/api/internal/mongodb/relation/stage_api", json={"stage_id": stage_id, "api_id": api_id, "position": 1}, headers=headers)
...
async def test_get_stage_api():
    async with get_db_client() as db:
        ...
        # Create a stage-api relation
        relation_id = str(ObjectId())
        await db.stage_api.insert_one({"_id": ObjectId(relation_id), "stage_id": stage_id, "api_id": api_id, "position": 1, "created_at": "2023-10-01T00:00:00Z", "created_by": user_id})

        response = client.get(f"/api/internal/mongodb/relation/stage_api/{relation_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["stage_id"] == stage_id
        assert data["api_id"] == api_id
        ...
        # Clean up
        await db.stage_api.delete_one({"_id": ObjectId(relation_id)})
        await db.stages.delete_one({"_id": ObjectId(stage_id)})
        await db.apis.delete_one({"_id": ObjectId(api_id)})
        await db.users.delete_one({"_id": ObjectId(user_id)})

@pytest.mark.asyncio
async def test_update_stage_api():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.put(f"/api/internal/mongodb/relation/stage_api/{relation_id}", json={"stage_id": stage_id, "api_id": api_id, "position": 2}, headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["stage_id"] == stage_id
        assert data["api_id"] == api_id
...
@pytest.mark.asyncio
async def test_delete_stage_api():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.delete(f"/api/internal/mongodb/relation/stage_api/{relation_id}", headers=headers)
        assert response.status_code == 200
        assert response.json() == {"message": "Stage-Api relation deleted successfully"}

        # Verify deletion
        relation = await db.stage_api.find_one({"_id": ObjectId(relation_id)})
