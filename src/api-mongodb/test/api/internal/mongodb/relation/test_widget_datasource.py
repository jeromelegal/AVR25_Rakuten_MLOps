...
@pytest.mark.asyncio
async def test_create_widget_datasource():
    async with get_db_client() as db:
        ...
        response = client.post("/api/internal/mongodb/relation/widget_datasource", json={"widget_id": widget_id, "datasource_id": datasource_id, "position": 1}, headers=headers)
...
async def test_get_widget_datasource():
    async with get_db_client() as db:
        ...
        # Create a widget-datasource relation
        relation_id = str(ObjectId())
        await db.widget_datasource.insert_one({"_id": ObjectId(relation_id), "widget_id": widget_id, "datasource_id": datasource_id, "position": 1, "created_at": "2023-10-01T00:00:00Z", "created_by": user_id})

        response = client.get(f"/api/internal/mongodb/relation/widget_datasource/{relation_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["widget_id"] == widget_id
        assert data["datasource_id"] == datasource_id
        ...
        # Clean up
        await db.widget_datasource.delete_one({"_id": ObjectId(relation_id)})
        await db.widgets.delete_one({"_id": ObjectId(widget_id)})
        await db.datasources.delete_one({"_id": ObjectId(datasource_id)})
        await db.users.delete_one({"_id": ObjectId(user_id)})

@pytest.mark.asyncio
async def test_update_widget_datasource():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.put(f"/api/internal/mongodb/relation/widget_datasource/{relation_id}", json={"widget_id": widget_id, "datasource_id": datasource_id, "position": 2}, headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["widget_id"] == widget_id
        assert data["datasource_id"] == datasource_id
...
@pytest.mark.asyncio
async def test_delete_widget_datasource():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.delete(f"/api/internal/mongodb/relation/widget_datasource/{relation_id}", headers=headers)
        assert response.status_code == 200
        assert response.json() == {"message": "Widget-Datasource relation deleted successfully"}

        # Verify deletion
        relation = await db.widget_datasource.find_one({"_id": ObjectId(relation_id)})
