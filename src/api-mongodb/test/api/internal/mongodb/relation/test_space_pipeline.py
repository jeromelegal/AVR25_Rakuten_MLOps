...
@pytest.mark.asyncio
async def test_create_space_pipeline():
    async with get_db_client() as db:
        ...
        response = client.post("/api/internal/mongodb/relation/space_pipeline", json={"space_id": space_id, "pipeline_id": pipeline_id, "position": 1}, headers=headers)
...
async def test_get_space_pipeline():
    async with get_db_client() as db:
        ...
        # Create a space-pipeline relation
        relation_id = str(ObjectId())
        await db.space_pipeline.insert_one({"_id": ObjectId(relation_id), "space_id": space_id, "pipeline_id": pipeline_id, "position": 1, "created_at": "2023-10-01T00:00:00Z", "created_by": user_id})

        response = client.get(f"/api/internal/mongodb/relation/space_pipeline/{relation_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["space_id"] == space_id
        assert data["pipeline_id"] == pipeline_id
        ...
        # Clean up
        await db.space_pipeline.delete_one({"_id": ObjectId(relation_id)})
        await db.spaces.delete_one({"_id": ObjectId(space_id)})
        await db.pipelines.delete_one({"_id": ObjectId(pipeline_id)})
        await db.users.delete_one({"_id": ObjectId(user_id)})

@pytest.mark.asyncio
async def test_update_space_pipeline():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.put(f"/api/internal/mongodb/relation/space_pipeline/{relation_id}", json={"space_id": space_id, "pipeline_id": pipeline_id, "position": 2}, headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["space_id"] == space_id
        assert data["pipeline_id"] == pipeline_id
...
@pytest.mark.asyncio
async def test_delete_space_pipeline():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.delete(f"/api/internal/mongodb/relation/space_pipeline/{relation_id}", headers=headers)
        assert response.status_code == 200
        assert response.json() == {"message": "Space-Pipeline relation deleted successfully"}

        # Verify deletion
        relation = await db.space_pipeline.find_one({"_id": ObjectId(relation_id)})
