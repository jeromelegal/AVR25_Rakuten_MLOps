...
@pytest.mark.asyncio
async def test_create_role_directory():
    async with get_db_client() as db:
        ...
        response = client.post("/api/internal/mongodb/relation/role_directory", json={"role_id": role_id, "directory_id": directory_id, "permissions": {"read": True, "write": False}}, headers=headers)
...
async def test_get_role_directory():
    async with get_db_client() as db:
        ...
        # Create a role-directory relation
        relation_id = str(ObjectId())
        await db.role_directories.insert_one({"_id": ObjectId(relation_id), "role_id": role_id, "directory_id": directory_id, "permissions": {"read": True, "write": False}, "created_at": "2023-10-01T00:00:00Z", "created_by": user_id})

        response = client.get(f"/api/internal/mongodb/relation/role_directory/{relation_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["role_id"] == role_id
        assert data["directory_id"] == directory_id
        ...
        # Clean up
        await db.role_directories.delete_one({"_id": ObjectId(relation_id)})
        await db.roles.delete_one({"_id": ObjectId(role_id)})
        await db.directories.delete_one({"_id": ObjectId(directory_id)})
        await db.users.delete_one({"_id": ObjectId(user_id)})

@pytest.mark.asyncio
async def test_update_role_directory():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.put(f"/api/internal/mongodb/relation/role_directory/{relation_id}", json={"role_id": role_id, "directory_id": directory_id, "permissions": {"read": True, "write": True}}, headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["role_id"] == role_id
        assert data["directory_id"] == directory_id
...
@pytest.mark.asyncio
async def test_delete_role_directory():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.delete(f"/api/internal/mongodb/relation/role_directory/{relation_id}", headers=headers)
        assert response.status_code == 200
        assert response.json() == {"message": "Role-Directory relation deleted successfully"}

        # Verify deletion
        role_directory = await db.role_directories.find_one({"_id": ObjectId(relation_id)})
        assert role_directory is None
