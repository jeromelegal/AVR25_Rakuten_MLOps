...
@pytest.mark.asyncio
async def test_create_role_datasource():
    async with get_db_client() as db:
        ...
        response = client.post("/api/internal/mongodb/relation/role_datasource", json={"role_id": role_id, "datasource_id": datasource_id, "permissions": {"read": True, "write": False}}, headers=headers)
...
async def test_get_role_datasource():
    async with get_db_client() as db:
        ...
        # Create a role-datasource relation
        relation_id = str(ObjectId())
        await db.role_datasources.insert_one({"_id": ObjectId(relation_id), "role_id": role_id, "datasource_id": datasource_id, "permissions": {"read": True, "write": False}, "created_at": "2023-10-01T00:00:00Z", "created_by": user_id})

        response = client.get(f"/api/internal/mongodb/relation/role_datasource/{relation_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["role_id"] == role_id
        assert data["datasource_id"] == datasource_id
        ...
        # Clean up
        await db.role_datasources.delete_one({"_id": ObjectId(relation_id)})
        await db.roles.delete_one({"_id": ObjectId(role_id)})
        await db.datasources.delete_one({"_id": ObjectId(datasource_id)})
        await db.users.delete_one({"_id": ObjectId(user_id)})

@pytest.mark.asyncio
async def test_update_role_datasource():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.put(f"/api/internal/mongodb/relation/role_datasource/{relation_id}", json={"role_id": role_id, "datasource_id": datasource_id, "permissions": {"read": True, "write": True}}, headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["role_id"] == role_id
        assert data["datasource_id"] == datasource_id
...
@pytest.mark.asyncio
async def test_delete_role_datasource():
    async with get_db_client() as db:
        # Create a base user
        user_id = str(ObjectId())
        hashed_password = hash_password("password")
        ...
        response = client.delete(f"/api/internal/mongodb/relation/role_datasource/{relation_id}", headers=headers)
        assert response.status_code == 200
        assert response.json() == {"message": "Role-Datasource relation deleted successfully"}

        # Verify deletion
        role_datasource = await db.role_datasources.find_one({"_id": ObjectId(relation_id)})
        assert role_datasource is None
