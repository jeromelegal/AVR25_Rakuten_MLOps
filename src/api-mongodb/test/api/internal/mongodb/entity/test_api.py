...
async def test_create_api():
    async with get_db_client() as db:
        ...
        response = client.post("/api/internal/mongodb/entity/api", json={"name": "newapi", "endpoint": "/newapi", "description": "New API description"}, headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == "newapi"
        assert data["endpoint"] == "/newapi"
        assert data["description"] == "New API description"
        assert "api_id" in data
        assert "created_at" in data

        # Clean up
        await db.apis.delete_one({"_id": ObjectId(data["api_id"])})
...
async def test_get_api():
    async with get_db_client() as db:
        ...
        response = client.get(f"/api/internal/mongodb/entity/api/{api_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == "testapi"
        assert data["endpoint"] == "/testapi"
...
async def test_update_api():
    async with get_db_client() as db:
        ...
        response = client.put(f"/api/internal/mongodb/entity/api/{api_id}", json={"name": "updatedapi", "endpoint": "/updatedapi", "description": "Updated API description"}, headers=headers)
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == "updatedapi"
        assert data["endpoint"] == "/updatedapi"
...
async def test_delete_api():
    async with get_db_client() as db:
        ...
        response = client.delete(f"/api/internal/mongodb/entity/api/{api_id}", headers=headers)
        assert response.status_code == 200
