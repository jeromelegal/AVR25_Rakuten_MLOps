...
async def create_ad_category(ad_category: AdCategory, current_user: dict = Depends(get_current_user)):
...
async def get_ad_category(relation_id: str, current_user: dict = Depends(get_current_user)):
    async with get_db_client() as db:
...
async def update_ad_category(relation_id: str, ad_category: AdCategory, current_user: dict = Depends(get_current_user)):
    async with get_db_client() as db:
        ad_category_dict = ad_category.model_dump()
...
async def delete_ad_category(relation_id: str, current_user: dict = Depends(get_current_user)):
    async with get_db_client() as db:
