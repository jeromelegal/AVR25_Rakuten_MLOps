...
from api.postgresql.relation.user_ads import router as user_ad_router
...
def test_app():
    ...
    app.include_router(user_ad_router)
...
@pytest.mark.asyncio
async def test_flow_user_ad(test_app):
    async with get_db_client(test_settings) as db:
        ...
        response = test_app.post("/api/internal/postgresql/relation/user_ad", json=new_user_ad, headers=headers)
        ...
        response = test_app.get(f"/api/internal/postgresql/relation/user_ad?ad_id={ad_id}&user_id={user_id}", headers=headers)
        print_response_details(response)
        assert response.status_code == 200
        ...
        response = test_app.delete(f"/api/internal/postgresql/relation/user_ad?ad_id={ad_id}&user_id={user_id}", headers=headers)
        print_response_details(response)
        assert response.status_code == 200
        assert response.json()["message"] == "User-Ad relation deleted successfully"

        # Cleanup
        await db.execute("DELETE FROM user_ads WHERE ad_id = $1 AND user_id = $2", ad_id, user_id)
