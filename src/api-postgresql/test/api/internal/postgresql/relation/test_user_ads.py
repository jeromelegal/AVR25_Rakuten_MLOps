...
async def test_create_ad_image(test_app):
    async with get_db_client(test_settings) as db:
        ...
        assert response.json()["user_id"] == new_ad_image["user_id"]
...
async def test_get_ad_image(test_app):
    async with get_db_client(test_settings) as db:
        ...
        response = test_app.get(f"/api/internal/postgresql/relation/ad_image?ad_id={ad_id}&user_id={user_id}", headers=headers)
        print_response_details(response)
        assert response.status_code == 200
...
async def test_delete_ad_image(test_app):
    async with get_db_client(test_settings) as db:
        ...
        response = test_app.delete(f"/api/internal/postgresql/relation/ad_image?ad_id={ad_id}&user_id={user_id}", headers=headers)
        print_response_details(response)
        assert response.status_code == 200
