...
from api.postgresql.relation.ads_cats import router as ads_cat_router
...
def test_app():
    ...
    app.include_router(ads_cat_router)
...
async def test_flow_ad_cat(test_app):
    async with get_db_client(test_settings) as db:
        ...
        response = test_app.post("/api/internal/postgresql/relation/ads_cats", json=new_ad_cat, headers=headers)
        ...
        response = test_app.get(f"/api/internal/postgresql/relation/ads_cats?cat_id={cat_id}&ad_id={ad_id}", headers=headers)
        print_response_details(response)
        assert response.status_code == 200
        ...
        response = test_app.delete(f"/api/internal/postgresql/relation/ads_cats?cat_id={cat_id}&ad_id={ad_id}", headers=headers)
        print_response_details(response)
        assert response.status_code == 200
        assert response.json()["message"] == "Ad-Cat relation deleted successfully"

        # Cleanup
        await db.execute("DELETE FROM ads_cats WHERE cat_id = $1 AND ad_id = $2", cat_id, ad_id)
