...
async def test_create_user():
    async with get_db_client() as conn:
        ...
        response = client.post("/api/internal/postgresql/entity/user", json={"username": "newuser", "email": "newuser@example.com", "password": "newpassword"}, headers=headers)
...
async def test_get_user():
    async with get_db_client() as conn:
        ...
        response = client.get(f"/api/internal/postgresql/entity/user/{user_id}", headers=headers)
        assert response.status_code == 200
        data = response.json()
...
async def test_update_user():
    async with get_db_client() as conn:
        ...
        response = client.put(f"/api/internal/postgresql/entity/user/{user_id}", json={"username": "updateduser", "email": "updateduser@example.com", "password": "newpassword"}, headers=headers)
        assert response.status_code == 200
        data = response.json()
...
async def test_delete_user():
    async with get_db_client() as conn:
        ...
        response = client.delete(f"/api/internal/postgresql/entity/user/{user_id}", headers=headers)
        assert response.status_code == 200
