...
class InfraDeployer:
    ...
    def check_prerequisites(self):
        ...
        # for dir_path in [ISO_DIR, VM_IMAGES_DIR, CLOUD_INIT_ISO_DIR, FILES_DIR]:
        ...
        if not os.path.exists(SHARED_DIR):
            ...
            # Récupère les permissions du répertoire
            mode = os.stat(SHARED_DIR).st_mode
            # Vérifie si les permissions sont exactement 666 (rw-rw-rw-)
    ...
    def wait_for_vm_shutdown(self, name: str, timeout: int = 3000, interval: int = 1):
        """Attend qu'une VM spécifique s'éteigne.
        Args:
            name: Nom de la VM à surveiller.
            timeout: Temps maximum d'attente en secondes (défaut: 300s / 5min).
            interval: Intervalle entre les vérifications en secondes (défaut: 5s).
        """
    ...
    def deploy_vms(self):
        ...
        for vm in self.infra['vms']:
            ...
            # Attendre que la VM s'éteigne d'elle-même
            self.wait_for_vm_shutdown(vm['name'])
            # Exécuter les scripts et copier les fichiers post-installation
            self.copy_files_to_vm(vm)
            self.run_post_install_scripts(vm)

    def run_post_install_scripts(self, vm: Dict):
        """Exécute des scripts déjà copiés dans la VM."""
        if 'post_install_scripts' not in vm:
            print(f"  ! Aucune post-installation définie pour {vm['name']}")
            return

        scripts = vm['post_install_scripts']
        if not isinstance(scripts, list):
            print(f"  ✗ Erreur: 'post_install_scripts' doit être une liste pour {vm['name']}")
            return

        disk_path = vm['cloud_init']['disks'][0]['path']

        print(f"  ✓ Exécution des scripts post-installation pour {vm['name']}...")
        try:
            for file_item in scripts:
                source_path = os.path.join(SCRIPTS_DIR, file_item)
                # Vérifier que le chemin est absolu (commence par /)
                if not source_path.startswith('/'):
                    print(f"  ✗ Erreur: Les chemins de scripts doivent être absolus (ex: /tmp/script.sh) pour {vm['name']}")
                    continue

                # Exécuter le script avec --run
                subprocess.run([
                    'virt-customize',
                    '-a', disk_path,
                    '--run', source_path,
                ...
                print(f"  ✓ Script {source_path} exécuté sur {vm['name']}")
        except subprocess.CalledProcessError as e:
            print(f"  ✗ Erreur lors de l'exécution des scripts pour {vm['name']}: {e}")



    def copy_files_to_vm(self, vm: Dict):
        """Copie des fichiers ou répertoires vers la VM en utilisant --copy-in."""
        if 'copy_files' not in vm:
            return

        files_to_copy = vm['copy_files']
        if not isinstance(files_to_copy, list):
            print(f"  ✗ Erreur: 'copy_files' doit être une liste pour {vm['name']}")
            return

        disk_path = vm['cloud_init']['disks'][0]['path']

        print(f"  ✓ Copie des fichiers vers {vm['name']}...")
        try:
            for file_item in files_to_copy:
                ...
                # Utiliser --copy-in pour copier des répertoires entiers
                subprocess.run([
                    'virt-customize',
                    '-a', disk_path,
                    '--copy-in', f'{source_path}:{file_item["destination"]}',
                    '--selinux-relabel'
                ], check=True)
                print(f"  ✓ {file_item['source']} copié vers {file_item['destination']}")
        except subprocess.CalledProcessError as e:
            print(f"  ✗ Erreur lors de la copie des fichiers pour {vm['name']}: {e}")
